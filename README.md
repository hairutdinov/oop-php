...
Способность объектов скрывать часть своего состояния и поведения от других объектов, предоставляя 
внешнему миру только определенный интерфейс взаимодействия с собой
...
Модель некоего объекта или явления реального мира, в которой опущены незначительные детали, 
не играющие существенной роли в данном контексте 
...
Показывает, что один класс скорее всего придется менять при изменении названия или сигнатуры методов
второго

Появляется там, где указываются конкретные названия классов - в вызовах конструктора, при указании
типов параметров и т.д.

Степень зависимости можно ослабить указав абстрактный класс или интерфейс.

#### Ассоциации

Когда один объект взаимодействует с другим

Как строгий вариант зависимости, при котором один объект всегда имеет доступ к объекту, с которым  
он взаимодействует
...
Специализированная разновидность ассоциации, которая описывает отношения один-ко-многим,
многие-ко-многим, часть-целое между _несколькими_ объектами

Обычно при таком виде связи один объект содержит другие, т.е. выступает контейнером или коллекцией
...
Графический язык для визуализации, специфицирования, конструирования и документирования артефактов 
систем, в которых главная роль принадлежит ПО

## Глава 12. Постоянные объекты: сериализация, маршалинг и реляционные базы данных

Концепция постоянства - концепция сохранения состояния объекта с целью его использования позднее

### Запись в реляционную базу данных

Open Database Connectivity (ODBC) - Открытый интерфейс взаимодействия с базами данных. 
ODBC драйверы абстрагируют определяемые поставщиками протоколы, обеспечивая общий интерфейс 
программирования приложения для клиентов баз данных. 
Создавая свои клиенты БД с использованием API-интерфейса ODBC, вы сделаете так, что ваши программы 
смогут получить доступ к большему количеству БД.
...
class A
{
    ...
    {
        return 'methodAA + ' . static::methodB();
    }
